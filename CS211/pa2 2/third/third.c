...
int solve (int** matrix) {
  int digit = 0; 
  // double loop to itterate through the 9x9 matrix
  for (int i = 0; i < 9; i++) {
    for (int j = 0; j < 9; j++) {
...
int main (int argc, char** argv) {
  ...
  // since this is a guessing algorithm, we will not have to check for if it is guessable or not so that method is not included in the third
