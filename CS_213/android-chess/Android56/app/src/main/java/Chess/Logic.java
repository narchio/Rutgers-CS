...
public class Logic {
    ...
    public static Cell getBlackKing(Cell[][] board) {
        ...
        Log.println(Log.ASSERT, "king", " the black king is null");
        return null;
    }
    /**
    ...
    public static Cell getWhiteKing(Cell[][] board) {
        for(int i = 0; i < board.length; i++) {
            for(int j = 0; j < board[0].length; j++) {
                if (board[i][j].getPiece() != null) {
                    Log.println(Log.ASSERT, "piece", "board[" + i + "][" + j + "]: " + board[i][j].getPiece().getPieceType());
                }
        ...
        Log.println(Log.ASSERT, "king", " the white king is null");
        return null;
    }
    /**
...
////                    }
////                }
////                if(startY+1 < board.length && board[startX][startY+1].getPiece() != null && board[startX][startY+1].getPiece().getPieceType() == Type.PAWN && board[startX][startY+1].getPiece().getColor() != board[startX][startY].getPiece().getColor()) {
////                    if(lmEndY == startY+1) {
////                        return true;
////                    }
////                }
...
//    }
    ...
    public static boolean isChecked(Cell[][] board, int x, int y) {
        ...
        //check down
        for(int i = 1; i < Math.abs(board.length-x); i++) {
            //there is a piece in the cell
            if(board[x+i][y].getPiece() != null) {
                //check if friendly piece
                if(board[x+i][y].getPiece().getColor() == board[x][y].getPiece().getColor()) {
        ...
        //check right
        for(int i = 1; i < Math.abs(board[0].length-y); i++) {
            //there is a piece in the cell
            if(board[x][y+i].getPiece() != null) {
                //check if friendly piece
                if(board[x][y+i].getPiece().getColor() == board[x][y].getPiece().getColor()) {
        ...
        xCopy = x-1;
        yCopy = y+1;
        while(xCopy >= 0 && yCopy < board[0].length) {
            //there is a piece in the cell
            if(board[xCopy][yCopy].getPiece() != null) {
                //check if friendly piece
                if(board[xCopy][yCopy].getPiece().getColor() == board[x][y].getPiece().getColor()) {
        ...
        xCopy = x+1;
        yCopy = y-1;
        while(xCopy < board.length && yCopy >= 0) {
            //there is a piece in the cell
            if(board[xCopy][yCopy].getPiece() != null) {
                //check if friendly piece
                if(board[xCopy][yCopy].getPiece().getColor() == board[x][y].getPiece().getColor()) {
        ...
        xCopy = x+1;
        yCopy = y+1;
        while(xCopy < board.length && yCopy < board.length) {
            //there is a piece in the cell
            if(board[xCopy][yCopy].getPiece() != null) {
                //check if friendly piece
                if(board[xCopy][yCopy].getPiece().getColor() == board[x][y].getPiece().getColor()) {
        ...
        if(board[x][y].getPiece().getColor() == Color.WHITE) {
            ...
            if(x-1 >= 0 && y-1 >= 0 && y+1 < board.length) {
        ...
        if(board[x][y].getPiece().getColor() == Color.BLACK) {
            //check the two directly above
            if(x+1 < board.length && y-1 >= 0 && y+1 < board.length) {
        ...
        if(x-2 >= 0 && x-2 < board.length && y-1 >= 0 && y-1 < board.length) {
        ...
        if(x-1 >= 0 && x-1 < board.length && y-2 >= 0 && y-2 < board.length) {
        ...
        for(Cell[] row: board) {
            for(Cell cell: row) {
                if(cell.getPiece() != null && cell.getPiece().getColor() == color) {
                    cell.getPiece().updateLegalMoves(board, cell.getX(), cell.getY());
    ...
    public static boolean checkMate(Cell[][] board, int x, int y) {
        //generate the moves
        ArrayList<Cell> kingMoves = board[x][y].getPiece().generateLegalMoves(board, x, y);
        if (kingMoves.size() > 0) {
            return false;
        }
