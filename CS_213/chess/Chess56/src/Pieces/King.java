...
/**
 * The class King represents a king chess piece
 * @author Nicolas Carchio
 * @author Adam Romano
 */
public class King extends Piece{
	/**
	 * Creates an instance of king with specified color and type KING
	 * @param color The color of the king
	 * @param type Type.KING
	 */
	...
	/**
	 * Generates a list of cells that represents the legal moves for a king.
	 * @param board The Chess board.
	 * @param start The cell containing the king.
	...
	public ArrayList<Cell> generateLegalMoves(Cell[][] board, Cell start) {
		...
		//check up-right
		if(startX-1 >= 0 && startY+1 < board[0].length-1) {
			temp.copyCell(board[startX-1][startY+1]);
			//check if the spot is free or if there is a piece to take
			if(board[startX-1][startY+1].getPiece() == null || (board[startX-1][startY+1].getPiece() != null && (board[startX-1][startY+1].getPiece().getColor() != board[startX][startY].getPiece().getColor()))) {
				//move the piece to check if it puts the king into check
		...
		//check down-left 
		if(startX+1 < board.length-1 && startY-1 >= 0) {
			temp.copyCell(board[startX+1][startY-1]);
			//check if the spot is free or if there is a piece to take
			if(board[startX+1][startY-1].getPiece() == null || (board[startX+1][startY-1].getPiece() != null && (board[startX+1][startY-1].getPiece().getColor() != board[startX][startY].getPiece().getColor()))) {
				//move the piece to check if it puts the king into check
		...
		//check down-right 
		if(startX+1 < board.length-1 && startY+1 < board[0].length-1) {
			temp.copyCell(board[startX+1][startY+1]);
			//check if the spot is free or if there is a piece to take
			if(board[startX+1][startY+1].getPiece() == null || (board[startX+1][startY+1].getPiece() != null && (board[startX+1][startY+1].getPiece().getColor() != board[startX][startY].getPiece().getColor()))) {
				//move the piece to check if it puts the king into check
		...
		if((startX == 0 && startY == 4) && board[startX][startY].getPiece().getNumMoves() == 0) {
			...
			if(board[0][0].getPiece().getPieceType() == Type.ROOK && board[0][0].getPiece().getColor() == board[startX][startY].getPiece().getColor() && board[0][0].getPiece().getNumMoves() == 0) {
			...
			if(board[0][7].getPiece().getPieceType() == Type.ROOK && board[0][7].getPiece().getColor() == board[startX][startY].getPiece().getColor() && board[0][7].getPiece().getNumMoves() == 0) {
				//check if there is a clear path 
		...
		if((startX == 7 && startY == 4) && board[startX][startY].getPiece().getNumMoves() == 0) {
			//try castling to the left 
			if(board[7][0].getPiece().getPieceType() == Type.ROOK && board[7][0].getPiece().getColor() == board[startX][startY].getPiece().getColor() && board[7][0].getPiece().getNumMoves() == 0) {
				//check if there is a clear path 
			...
			//try castling to the right
			if(board[7][7].getPiece().getPieceType() == Type.ROOK && board[7][7].getPiece().getColor() == board[startX][startY].getPiece().getColor() && board[7][7].getPiece().getNumMoves() == 0) {
				//check if there is a clear path 
